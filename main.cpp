...
int example() {
  ...
  // Materials
  std::shared_ptr<m::Material> billiardMat = std::make_shared<m::GeneralMaterial>(
  ...
  std::shared_ptr<m::Material> grayMat = std::make_shared<m::GeneralMaterial>(
      c::SColor({0.5, 0.5, 0.5}), c::SColor({0.5, 0.5, 0.5}), 128.0,
  ...
  scene.primitives.emplace_back(std::make_shared<m::Sphere>(
      geometry::Point3D{-2.0, -2.5, -8.0}, 1.5, g::RotateY3D(-0.2), earthMat, earthNormal));
  
    scene.primitives.emplace_back(std::make_shared<m::Sphere>(
        g::Point3D{0.2, -2.5, -6.5}, 1.5, g::Identity3D(), glassMat, normalDrops));

    scene.primitives.emplace_back(std::make_shared<m::Sphere>(
        g::Point3D{1.0, -2.5, -10.0}, 1.5, g::Identity3D(), mirrorMat));

    scene.primitives.emplace_back(
        std::make_shared<m::Sphere>(g::Point3D{3.3, -2.8, -6.0}, 1.2, g::RotateX3D(-0.7)*g::RotateY3D(1.3), billiardMat));

    scene.primitives.emplace_back(std::make_shared<m::Sphere>(
        g::Point3D{-5.0, -2.8, -10.0}, 1.2, g::Identity3D(), mirrorMat, waterNormal));

    scene.primitives.emplace_back(std::make_shared<m::Sphere>(
        g::Point3D{3.6, -2.8, -9.0}, 1.2, g::RotateY3D(0.5)*g::RotateZ3D(0.5), ballMat, ballNormal));
  ...
  color::ImageSize imageSize{320, 180};  //{640, 480};
